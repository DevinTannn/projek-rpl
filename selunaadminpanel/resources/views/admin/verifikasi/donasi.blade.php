...
<div class="p-4 lg:p-8">
    <div class="flex flex-col gap-4 lg:flex-row lg:justify-between lg:items-center mb-6">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            {{-- Search Bar --}}
            <form action="{{ route('admin.verify.donation.index') }}" method="GET" class="flex items-center gap-2 bg-white px-3 py-2 rounded-xl shadow-sm border border-gray-100">
                ...
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari donatur / kampanye..." class="text-xs font-medium text-gray-700 bg-transparent border-none focus:outline-none focus:ring-0 w-44" autocomplete="off">
                @if(!empty($search))
                    <a href="{{ route('admin.verify.donation.index') }}" class="text-gray-400 hover:text-red-500 transition text-xs" title="Hapus pencarian">✕</a>
                @endif
                ...
                <tr class="hover:bg-gray-50 transition-colors {{ $d->status == 'pending' ? 'bg-amber-50/30' : '' }}">
                    ...
                    <td class="px-6 py-4 text-right">
                        ...
                        <div class="flex justify-end gap-2">
                            <form action="{{ route('admin.verify.donation.update', $d->id) }}" method="POST" data-confirm data-confirm-title="Approve Donasi?" data-confirm-body="Anda akan menyetujui donasi manual ini. Donasi akan ditandai sebagai terbayar." data-confirm-icon="✅" data-confirm-ok="Ya, Approve">
                                @csrf
                            ...
                            <form action="{{ route('admin.verify.donation.update', $d->id) }}" method="POST" data-confirm data-confirm-title="Reject Donasi?" data-confirm-body="Anda akan menolak donasi manual ini. Tindakan ini tidak bisa dibatalkan." data-confirm-icon="❌" data-confirm-ok="Ya, Reject">
                                @csrf
